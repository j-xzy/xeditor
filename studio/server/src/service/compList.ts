...
interface IProps {
  src: string;
  dist: string;
  mapPath: (name: string, version: string) => string;
}
...
export async function oneUserCompList({ src, dist, mapPath }: IProps): Promise<IComp[] | null> {
  try {
    const comps: Array<{ name: string, path: string, img: string }> = [];
...
export async function allUserCompList(mapPath: (username: string, name: string, version: string) => string) {
  ...
  const allComps: Array<{ type: string; comps: IComp[] }> = [];
