...
function RawDoubleInput(props: IProps) {
  ...
  return (
    <div className='layer-dbipt'>
      ...
      <input className='layer-dbipt-ipt layer-ipt' onKeyDown={(e) => e.key === 'Enter' && iptRef.current!.blur()} onChange={props.onChange} hidden={disabled} ref={iptRef} type='text' tabIndex={0} value={props.value} onBlur={() => setDisabled(true)} />
    </div>
  );
}
