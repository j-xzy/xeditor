import { ILayerPluginState } from '@xeditor/plugin-layer';
...
const Target = {
  hover(props: IProps, monitor: DropTargetMonitor) {
    ...
    if (props.node.State.id === (monitor.getItem().node as Editor.IFeature).State.id) {
  ...
  drop(props: IProps, monitor: DropTargetMonitor) {
    ...
    const sourceId = (monitor.getItem().node as Editor.IFeature).State.id!;
...
function RawNode(props: IProps & ISourceCollectedProps & ITargetCollectedProps) {
  ...
  React.useEffect(() => {
    ...
    // 不能使用useEffect(,[]),以为node会发生改变
  });

  const state = node.State;
  const isSelected = layer.selectedId === props.node.State.id;
  const isEyeClosed = typeof state.style !== 'undefined' && state.style.visibility === 'hidden';
  const children = node.getChildren();

  return props.connectDropTarget(
    props.connectDragSource((
      <div className='layer-node'>
        <div id={`layer-node-${state.id}`} className={`layer-node-item ${isSelected ? 'layer-node-highlight' : ''}`} onClick={() => layer.updateSelectedFtr(state.id!)} >
