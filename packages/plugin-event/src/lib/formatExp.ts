import { BaseNode, Expression, PropertyExpression, WithTreeNode } from '../node';
...
export function formatExp(expParent: Editor.ITreeNode) {
  ...
  for (let i = 1; i < expres.length; i++) {
    ...
    if (typeof last.value !== 'string' && typeof next.value !== 'string') {
      last.insertAsSuccessor(Factory.Instance.EmptyExpression(last.plugin));
    }
  }

  if (typeof expres[expres.length - 1].value !== 'string') {
    expres[expres.length - 1].insertAsSuccessor(Factory.Instance.EmptyExpression(expres[expres.length - 1].plugin));
  }
}
